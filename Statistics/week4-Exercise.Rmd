...
filter(gapminder, (country == "United Kingdom" | country == "Vietnam") & year > 1960)

gapminder %>% arrange(desc(gdpPercap)) %>% slice(1:15)
gapminder %>% mutate(gdpPercap_lifeExp = gdpPercap / lifeExp) %>% select(gdpPercap, lifeExp, gdpPercap_lifeExp)
gapminder %>% mutate(gdpPercap_lifeExp = gdpPercap / lifeExp) %>% summarise(avgGdpLifExp = mean(gdpPercap_lifeExp))
compare.lifeExp <- gapminder %>% group_by(continent) %>% summarise(avgLifeExp = mean(lifeExp))
...
arrange(mlb16.data, desc(HR))
arrange(mlb16.data, desc(RBI))
mlb16.data %>% group_by(Lg) %>% summarise(avgRBI = mean(RBI))
